...
assert_contains "$install_sh" 'git -C "\$REPO_DIR" restore --source=HEAD --worktree' 'installer must restore missing tracked assets before copying'
...
assert_contains "$install_sh" 'MIN_FREE_MB=256' 'installer must keep a small disk-space safety margin'
...
assert_contains "$install_sh" 'fastfetch-linux-amd64' 'fastfetch must use the Ubuntu-compatible glibc build'
assert_not_contains "$install_sh" 'fastfetch-musl-amd64' 'fastfetch must not require the unavailable musl loader'
assert_contains "$install_sh" 'extract-release-binary\.py' 'safe release extractor must be used'
assert_contains "$install_sh" '"\$STAGED_BIN/\$binary_name" --version' 'every downloaded CLI binary must execute before activation'
assert_contains "$install_sh" 'version_output=' 'binary validation failures must preserve diagnostic output'
assert_contains "$install_sh" 'if ! curl -fsSL "\$url"' 'download failure must stop staging explicitly'
assert_contains "$install_sh" 'if ! printf .*sha256sum -c' 'checksum failure must stop staging explicitly'
assert_contains "$install_sh" 'if ! python3 "\$REPO_DIR/scripts/extract-release-binary.py"' 'extraction failure must stop staging explicitly'
assert_contains "$install_sh" '\[ ! -x "\$STAGED_BIN/\$tool" \]' 'activation must skip missing staged binaries'
assert_contains "$install_sh" 'Keeping existing executable:' 'failed staging must preserve the current installed tool'
assert_contains "$install_sh" 'if mv -f "\$candidate" "\$target"' 'atomic replacement failure must be checked explicitly'
...
assert_contains "$apply_gnome_sh" 'APPLY_WALLPAPER:-0' 'wallpaper changes must be off by default'
assert_contains "$apply_gnome_sh" 'APPLY_WALLPAPER:-0\}" = "1"' 'wallpaper changes must require APPLY_WALLPAPER=1 opt-in'
